...
remove_duplicates_entity$used(pre_sel_grids_entity)
...
for (j in 1:nrow(subset)) {
  ...
  if (cell$cell_ID %in% tile) {
    ...
    sel_least_cloudy_img_activity <- Activity("sel_least_cloudy_img", "select least cloudy image")
    sel_least_cloudy_img_activity$used(cleaned_grids_entity)

    least_cloudy_img_entity <- Entity("least_cloudy_img", "least cloudy image of the year")
    least_cloudy_img_entity$wasGeneratedBy(sel_least_cloudy_activity)
    ...
    try(
      {
        ...
        mask_out_clouds_activity <- Activty("mask_out_clouds_and_shadows", "mask out clouds and shadows")
        mask_out_clouds_activity$used(least_cloudy_img_entity)

        processed_landsat_img_entity <- Entity("processed_landsat_img", "processed landsat image")
