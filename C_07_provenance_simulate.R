...
library(devtools)
load_all()
init_provenance_graph(namespace = "https://www.idiv.de/de/luckinet.html/geokur/C07#")
...
pre_sel_grids_entity <- Entity(id = "pre_selected_grids", label = "pre selected grids")
remove_duplicates_activity <- Activity(id = "remove_duplicated", label = "remove duplicates")
remove_duplicates_activity$used(pre_sel_grids_entity)

cleaned_grids_entity <- Entity("cleaned_grids", "landsat image collection", description = "yearly landsat image collection for each grid cell")
cleaned_grids_entity$wasGeneratedBy(remove_duplicates_activity)
...
for (j in 1:10) {
  ...
  if (0) {
    ...
    # build new process node in each loop, but give the same label
    # semantically that means something like: it is the same process, but different instantiations
    id <- paste("sel_least_cloudy_img", j, sep = "_")
    sel_least_cloudy_img_activity <- Activity(id, "select least cloudy image")
    sel_least_cloudy_img_activity$used(cleaned_grids_entity)

    id <- paste("least_cloudy_img", j, sep = "_")
    label <- paste("least cloudy image of the year", j, sep = " ")
    least_cloudy_img_entity <- Entity(id, label)
    least_cloudy_img_entity$wasGeneratedBy(sel_least_cloudy_img_activity)
    ...
    try(
      {
        ...
        id <- paste("mask_out_clouds_and_shadows", j, sep = "_")
        mask_out_clouds_activity <- Activity(id, "mask out clouds and shadows")
        mask_out_clouds_activity$used(least_cloudy_img_entity)

        id <- paste("processed_landsat_img", j, sep = "_")
        label <- paste("processed landsat image", j, sep = " ")

        processed_landsat_img_entity <- Entity(id, label)
        processed_landsat_img_entity$wasGeneratedBy(mask_out_clouds_activity)
      },
      silent = F
    )
  }
...
serialize_provenance_graph(name = "catarina.ttl")